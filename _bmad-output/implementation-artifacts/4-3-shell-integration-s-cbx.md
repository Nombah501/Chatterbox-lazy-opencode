...
- [ ] Task 4: Smoke-проверка интеграции
  - [ ] Subtask 4.1: В OpenCode убедиться, что при `session.idle` запускается cbx
  - [ ] Subtask 4.2: Проверить негативный сценарий (cbx отсутствует) — лог без падения
...
- 2026-01-07: Тест cbx `--tts --tts-force` завершился без аудиофайла; требуется исправление TTS выполнения.
